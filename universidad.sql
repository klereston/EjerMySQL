...
select nombre, apellido1, telefono, nif from persona
...
select DISTINCT departamento.nombre as departamentos_gradoIngenieriaInformatica_conProfesor from profesor
INNER JOIN departamento
ON id_departamento = departamento.id
where profesor.id_profesor IN (select id_profesor from asignatura
INNER JOIN grado
ON id_grado = grado.id
where grado.nombre = 'Grado en Ingeniería Informática (Plan 2015)' AND id_profesor IS NOT NULL);
/*9.Retorna un llistat amb tots els alumnes que s'han matriculat en alguna assignatura durant el curs escolar 2018/2019.*/
select DISTINCT persona.*, curso_escolar.anyo_inicio as curso_escolar from alumno_se_matricula_asignatura
INNER JOIN persona
ON id_alumno = persona.id
INNER JOIN asignatura
ON id_asignatura = asignatura.id
INNER JOIN curso_escolar
ON id_curso_escolar = curso_escolar.id
where curso_escolar.anyo_inicio between '2018' and '2019';
...
El llistat ha de retornar quatre columnes, nom del departament, primer cognom, 
segon cognom i nom del professor/a. El resultat estarà ordenat alfabèticament de menor a major pel nom del departament, cognoms i el nom.*/
...
select departamento.nombre as departamento, id_profesor from profesor
RIGHT JOIN departamento 
ON id_departamento = departamento.id
LEFT JOIN persona
ON id_profesor = persona.id
...
select persona.*, asignatura.id as asignatura from asignatura
RIGHT JOIN profesor
...
select asignatura.*, profesor.id_profesor as profesor from asignatura
LEFT JOIN profesor
ON asignatura.id_profesor = profesor.id_profesor
...
El resultat només ha d'incloure els departaments que tenen professors/es 
associats i haurà d'estar ordenat de major a menor pel nombre de professors/es.*/
select COUNT(profesor.id_profesor), departamento.nombre as departamento from profesor
INNER JOIN departamento
ON id_departamento = departamento.id
...
select departamento.nombre as departamento, COUNT(profesor.id_profesor) as profesores from profesor
RIGHT JOIN departamento
ON id_departamento = departamento.id
GROUP BY departamento.id
ORDER BY COUNT(profesor.id_profesor) DESC;
...
select grado.nombre as grado, COUNT(asignatura.id) from asignatura
RIGHT JOIN grado
...
select grado.nombre as grado, asignatura.tipo, COUNT(asignatura.creditos) from asignatura
LEFT JOIN grado
ON grado.id=asignatura.id_grado
...
select curso_escolar.anyo_inicio, COUNT(id_alumno) FROM alumno_se_matricula_asignatura
...
select * from persona
where fecha_nacimiento in (select max(fecha_nacimiento) from persona) and tipo = 'alumno'; 
